...
public class LiquidGenerator : MonoBehaviour
{
    ...
    public List<Liquid> GeneratorLiquid(int[,] tileMapping, int sectionWidth, int mapLength, int sectionHeight, GameObject liquid, int sectionNumber)
    {
            ...
            for (int y = 0; y < sectionHeight; y++)
            {
                ...
                if (tileMapping[x, y] == 0 && tileMapping[x - 1, y] == 1)
                {
                    ...
                    if (firstLand > x + 1 && deepestLand > y + 1 )
                    {
                        ...
                        {   if (tempLiquids[q].x < x + 1 && firstLand > x + 1)
                                if (tempLiquids[q].y < y && deepestLand > y)
                                    cantPlace = true;
                        }
                        if (!cantPlace)
                        {
                            Liquid newLiquid = new Liquid(new Vector3(x - sectionWidth  + this.initialX + (this.initialX/2), this.initialY - y), x-firstLand, y- deepestLand, liquid);
